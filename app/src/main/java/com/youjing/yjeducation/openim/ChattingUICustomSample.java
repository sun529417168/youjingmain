package com.youjing.yjeducation.openim;
...
//        }
    ...
    public View getCustomTitleView(final Fragment fragment,
        ...
        View view = inflater.inflate(R.layout.demo_custom_chatting_titl, new RelativeLayout(context), false);

       // view.setBackgroundColor(Color.parseColor("#0caafe"));
        TextView textView = (TextView) view.findViewById(R.id.title);



        RelativeLayout btn_delete = (RelativeLayout) view.findViewById(R.id.btn_delete);
        final RelativeLayout re_notice = (RelativeLayout) view.findViewById(R.id.re_notice);
        TextView txt_content = (TextView) view.findViewById(R.id.txt_content);
        if(YJGlobal.getYjimGroupModel() != null){
            String aliGroupNotice = YJGlobal.getCustomerId()+YJGlobal.getYjimGroupModel().getAliGroupId()+YJGlobal.getYjimGroupModel().getAliGroupNoticeId() + "YJ";
             isClose =  SharedUtil.getBoolean(YJApplication.getContext(), aliGroupNotice, false);
            txt_content.setText(YJGlobal.getYjimGroupModel().getAliGroupNotice());
        }
        if(YJGlobal.getNoticeIsNew()){
        ...
        btn_delete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                // Notification.showToastMsgLong(fragment.getActivity(), "这是公告内容");

                if(YJGlobal.getYjimGroupModel() != null){
                    String aliGroupNotice = YJGlobal.getCustomerId()+YJGlobal.getYjimGroupModel().getAliGroupId()+YJGlobal.getYjimGroupModel().getAliGroupNoticeId()+"YJ";
                    SharedUtil.setBoolean(YJApplication.getContext(), aliGroupNotice, true);
                }
        ...
        if (conversation.getConversationType() == YWConversationType.P2P) {
            ...
            if (conversation.getConversationBody() instanceof YWTribeConversationBody) {
                ...
                if (TextUtils.isEmpty(title)) {
                    if (!TextUtils.isEmpty(YJGlobal.getMy_grade()) && !TextUtils.isEmpty(YJGlobal.getMy_subject())){
                        title = YJGlobal.getMy_grade()+YJGlobal.getMy_subject();
                    }
...
//                + "/alibaba/WXOPENI/云旺相册";
    ...
    public View getChattingFragmentCustomViewAdvice(Fragment fragment, Intent intent) {
        ...
        if (intent != null && intent.hasExtra("extraTribeId") && intent.hasExtra("conversationType")) {
            ...
            if (tribeId > 0 && conversationType == YWConversationType.Tribe.getValue()) {
                YWIMKit mIMKit = YJGlobal.getmIMKit();
                if (mIMKit == null) {
                    return null;
                }
    ...
    private void receiveTribeMsg(YWTribe tribe, final View view) {
        YJGlobal.getmIMKit().getIMCore().getTribeService().unblockTribe(tribe, new IWxCallback() {
            @Override
    ...
    public List<YWInputViewPlugin> adjustCustomInputViewPlugins(final Fragment fragment, YWConversation conversation, List<YWInputViewPlugin> pluginItems) {
        if (pluginItems != null && pluginItems.size() > 0) {
            ...
            for (YWInputViewPlugin plugin : pluginItems) {
                if (plugin.getId() == YWChattingPlugin.ReplyBarPlugin.FACE_VIEW) {
                    plugin.setNeedHide(false);//隐藏语音输入按钮
                }
            }
        }
